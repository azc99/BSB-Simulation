...
c     6x6, columns mirrored from table
     ...
     .        data_interval,spin_order(j,j),spin_order2(j,j)
...
c--------------------------------------
      ...
      do x = 0,2**(3*j)-1
          ...
          xx = IOR(xx,ISHFT(s(spin_order(m+1,1)),5))
          xx = IOR(xx,ISHFT(s(spin_order(m+1,2)),4))
          xx = IOR(xx,ISHFT(s(spin_order(m+1,3)),3))
          xx = IOR(xx,ISHFT(s(spin_order(m+1,4)),2))
          xx = IOR(xx,ISHFT(s(spin_order(m+1,5)),1))
          xx = IOR(xx,s(spin_order(m+1,6)))
...
C$OMP& PRIVATE(ts,t2,x,ii,PROB,s,m1,m2,m3,m4,m5,m6,xx,curp)
     ...
     .                  w,wqbt,w_k,spin_order)
         CALL spin_mode(j,k,m,curp,A,B,t2,t1,LD,RABI,
     .                  w,wqbt,w_k,spin_order,spin_order2,swap)          
...
C$OMP DO SCHEDULE(STATIC)
            do x = 0,2**(3*j)-1
               ...
               xx = 0
               xx = IOR(xx,ISHFT(s(spin_order(m+1,1)),17))
               xx = IOR(xx,ISHFT(s(spin_order(m+1,2)),16))
               xx = IOR(xx,ISHFT(s(spin_order(m+1,3)),15))
               xx = IOR(xx,ISHFT(s(spin_order(m+1,4)),14))
               xx = IOR(xx,ISHFT(s(spin_order(m+1,5)),13))
               xx = IOR(xx,ISHFT(s(spin_order(m+1,6)),12))
...
C===================================================================
     ...
     .  (-i*plusx-minusy)
      ...
      if (cur .EQ. 1) then
          CALL x_spin_only(z,A,B,SIG)
      else
          CALL x_spin_only(z,B,A,SIG)
      end if
      cur = IEOR(cur,1)
     ...
     .(-i*plusx-minusy)
      ...
      if (cur .EQ. 1) then
         CALL to_spin_mode(z,A,B,SIG)
      else
         CALL to_spin_mode(z,B,A,SIG)
      end if
      cur = IEOR(cur,1)
      ...
      subroutine x_spin_only(z,A,B,SIG)
...
C$OMP END DO
      ...
      subroutine to_spin_mode(z,A,B,SIG)
      INTEGER z,x,s_e,s_d,s_c,s_b,s_a,m6,m5,m4,m3,m2,m1,state,xx
      COMPLEX*16 A(0:262143),B(0:262143),OUTP(0:1),SIG(0:1,0:1)
      COMPLEX*16 a0,a1
      
C$OMP DO SCHEDULE(STATIC)
C$OMP& PRIVATE(x,a0,a1,OUTP,state,xx,s_a,s_b,s_c,s_d,s_e)
C$OMP& PRIVATE(m1,m2,m3,m4,m5,m6)
      do x=0,131071
         ...
         xx = 0
         xx = IOR(xx, ISHFT(s_e,17))
         xx = IOR(xx, ISHFT(m6,15))
c         xx = IOR(xx, ISHFT(s_f,14))
         xx = IOR(xx, ISHFT(m5,12))
         xx = IOR(xx, ISHFT(s_a, 11))
         xx = IOR(xx, ISHFT(m4, 9))
         xx = IOR(xx, ISHFT(s_b,8))
         xx = IOR(xx, ISHFT(m3,6))
         xx = IOR(xx, ISHFT(s_c,5))
