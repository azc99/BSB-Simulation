      subroutine specific_time(j,k,tau,m,LD,del_t,points,MAG_C,
     .                         obj_sum,pi_times,w_access,detune)
      IMPLICIT NONE
     ...
     .                 SPIN(j,2),tau,P(j),PROB
      INTEGER low,high,driving_mode,cnt,points,
     .        data_interval
      DOUBLE PRECISION del_t,t1,t2,detune(7,7),obj_sum
      DOUBLE PRECISION MAG_C(0:points,0:j*k),pi_times(7)
      DOUBLE PRECISION RABI(7),w_k(7),w(7),wqbt(7),w_access(7)
      CHARACTER*50 F1,F2,F3
      INTEGER s1,s2,s3,s4,s5,s6,m1,m2,m3,m4,m5,m6,m7,xx,cur,curp
...
c--------------------------------------
      ...
      do x=1,j
         ...
         obj_sum = obj_sum + abs(PROB-MAG_C(cnt,m*j+x))**2
      end do
      cnt = cnt + 1
      ...
      do x = 0,2**(3*j)-1
          ...
          s6 = IAND(ISHFT(x,-12),1)
          s5 = IAND(ISHFT(x,-13),1)
          s4 = IAND(ISHFT(x,-14),1)
          s3 = IAND(ISHFT(x,-15),1)
          s2 = IAND(ISHFT(x,-16),1)
          s1 = IAND(ISHFT(x,-17),1)

          xx = 0
          xx = IOR(xx,ISHFT(m1,16))
          xx = IOR(xx,ISHFT(m2,14))
          xx = IOR(xx,ISHFT(m3,12))
          xx = IOR(xx,ISHFT(m4,10))
          xx = IOR(xx,ISHFT(m5,8))
          xx = IOR(xx,ISHFT(m6,6))
...
C$OMP& PRIVATE(ts,t2,x,ii,PROB,s1,s2,s3,s4,s5,s6,
C$OMP&         m1,m2,m3,m4,m5,m6,xx,curp)
...
C$OMP DO SCHEDULE(STATIC)
            do x = 0,2**(3*j)-1
               ...
               m6 = IAND(ISHFT(x,-6),3)   
               m5 = IAND(ISHFT(x,-8),3)   
               m4 = IAND(ISHFT(x,-10),3)
               m3 = IAND(ISHFT(x,-12),3)
               m2 = IAND(ISHFT(x,-14),3)
               m1 = IAND(ISHFT(x,-16),3)

               xx = 0
               xx = IOR(xx,ISHFT(s1,17))
               xx = IOR(xx,ISHFT(s2,16))
               xx = IOR(xx,ISHFT(s3,15))
               xx = IOR(xx,ISHFT(s4,14))
               xx = IOR(xx,ISHFT(s5,13))
               xx = IOR(xx,ISHFT(s6,12))
...
C$OMP SINGLE
            ...
            do ii = 1, j
                ...
                obj_sum = obj_sum + abs(PROB-MAG_C(cnt,m*j+ii))**2
            end do
            cnt = cnt + 1
C$OMP END SINGLE
         end if
...
C=================================================================
  ...
  100 CONTINUE
      ...
      INTEGER z,x,s_swap,diff,xx
      COMPLEX*16 A(0:262143),B(0:262143),OUTP(0:1),hc1,hc2
      COMPLEX*16 a0,a1
C$OMP DO PRIVATE(x,a0,a1,OUTP,s_swap,diff,xx)
C$OMP& SCHEDULE(STATIC)
...
C$OMP END DO
      ...
      INTEGER z,x,s6,s5,s4,s3,s2,m6,m5,m4,m3,m2,m1,state,xx
      COMPLEX*16 A(0:262143),B(0:262143),OUTP(0:1),hc1,hc2
      COMPLEX*16 a0,a1
...
C$OMP& PRIVATE(s2,s3,s4,s5,m1,m2,m3,m4,m5)
      do x=0,131071
         ...
         m6 = IAND(ISHFT(state, -6), 3)              
         m5 = IAND(ISHFT(state, -8), 3)
         m4 = IAND(ISHFT(state, -10), 3)
         m3 = IAND(ISHFT(state, -12), 3)
         m2 = IAND(ISHFT(state, -14), 3)
         m1 = IAND(ISHFT(state, -16), 3)
...
C$OMP END DO
      ...
      COMPLEX*16 A(0:262143),B(0:262143),EVEN(0:7,0:7),ODD(0:7,0:7),
...
C$OMP END DO
     ...
     .     IOR(ISHFT(diff,3*y+2),ISHFT(diff,2)))
      ...
      INTEGER x,xx,y,state,m1,m2,m3,m4,m5,m6,s1,s2,s3,s4,s5,s6
...
c==== create psi input vector      
     ...
     .     bi*2**(2*k+(j-ion))
      ...
      subroutine run_model(j,k,LD,MAG_C,obj_sum,config,points,
     .                     tau,w_access,pi_times,detune)
        
      implicit none
      INTEGER j,k,config,points,data_interval
      double precision LD(j,k),MAG_C(0:points,0:j*k)
      double precision tau,del_t,pi_times(7),w_access(7),detune(7,7)
      double precision, intent(out) :: obj_sum
     ...
     .                   points,MAG_C,obj_sum,pi_times,w_access,detune)
