      subroutine specific_time(j,k,tau,m,LD,del_t,points,
     .                         pi_times,w_access,detune)
      IMPLICIT NONE
     ...
     .        data_interval
      DOUBLE PRECISION del_t,t1,t2,detune(j,j),pi_times(7)
...
c--------------------------------------
      ...
      open(unit=11, file=F4, status='replace',
     .     form='formatted',recl=2048)
      ...
      if (m .EQ. 0) then
         write(11,'(8(1PE25.16,1X))')
     .        t1,(P(x),x=1,j)
      else
         write(11,'(7(1PE25.16,1X))')
     .        (P(x),x=1,j)
      end if
      flush(11)
