      subroutine specific_time(j,k,tau,m,LD,del_t,
     .                   points,MAG_C,obj_sum,pi_times,w_access,detune)
      IMPLICIT NONE
      INTEGER j,k,x,y,z,ii,iii,unit_num,m,ts,total_ts
      COMPLEX*16 PSI_IN(0:2**j*4**k-1),A(0:2**j*4**k-1),
     .           B(0:2**j*4**k-1),i
      DOUBLE PRECISION LD(j,k),pi,MODE(k,4),
     .                 SPIN(j,2),tau,P(7),PROB
      INTEGER low,high,driving_mode,cnt,
     .        data_interval, points
      DOUBLE PRECISION del_t,t1,t2,obj_sum
      DOUBLE PRECISION pi_times(7),RABI(7),w_k(7),w(7),wqbt(7)
      DOUBLE PRECISION MAG_C(0:points,0:j*k),w_access(7),detune(7,7)
      CHARACTER*30 F1,F2,F3
...
c==== create psi input vector      
     ...
     .                     tau,w_access,pi_times,detune)
      ...
      double precision tau,del_t,P1,P2,P3,take_data,pi_times(7)
