...
if os.name == "nt":
    raise RuntimeError(
        "fortran_module.py must be run using Linux Python inside WSL. "
        "The GPU library is a Linux .so file, not a Windows .dll."
    )


_module_dir = Path(__file__).resolve().parent
_lib_path = _module_dir / "libfortran_model.so"

if not _lib_path.exists():
    raise FileNotFoundError(
        f"Fortran shared library not found: {_lib_path}\n"
        "Compile sim_66.f with nvfortran before importing this module."
    )


try:
    # RTLD_GLOBAL helps make the NVIDIA/OpenMP runtime symbols available
    # if multiple nvfortran-built shared libraries are loaded.
    _lib = ctypes.CDLL(
        str(_lib_path),
...
_run_model.argtypes = [
    ...
    ctypes.POINTER(ctypes.c_double),                           # sum output
...
def run_model(
    ...
    """Run one GPU-accelerated simulation configuration."""
    ...
    sum_value = ctypes.c_double(0.0)

    ld_f = np.asfortranarray(ld, dtype=np.float64)
    mag_c_f = np.asfortranarray(mag_c, dtype=np.float64)
    ...
    expected_mag_shape = (points + 1, j * k + 1)
    if mag_c_f.shape != expected_mag_shape:
        raise ValueError(
            f"MAG_C must have shape {expected_mag_shape}, "
            f"got {mag_c_f.shape}"
        )

    # Your Fortran interface uses fixed-size seven-element buffers,
    # even for the 6-ion/6-mode case.
    ...
    result = float(sum_value.value)

    if not np.isfinite(result):
        raise FloatingPointError(
            f"Fortran run_model returned a non-finite value: {result}"
        )
